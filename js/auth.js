...
function updateAuthUI() {
    ...
    if (isLoggedIn && currentUser) {
        authStatusContainer.innerHTML = `
            <span class="welcome-message">Welcome, <span class="username">${currentUser.username}</span>!</span>
...
async function handleLogout() { try { await fetch('/api/users/logout', { method: 'POST' }); isLoggedIn = false; currentUser = null; updateAuthUI(); location.hash = '#'; location.reload(); } catch (error) { console.error('Logout failed:', error); } }
