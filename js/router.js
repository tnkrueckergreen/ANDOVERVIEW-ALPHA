...
export async function handleRouteChange() {
    ...
    // Store current location when navigating to auth pages
    ...
    if (path === 'login' || path === 'signup') {
        ...
        // Store the current hash for potential auth redirects
        sessionStorage.setItem('previousHash', currentHash);
    }

    if (footerCTA) {
        const pagesToHideFooterOn = ['subscribe', 'login', 'signup', 'contact'];
