 ...
 * @param {Object} options - Display options { variant: 'full' | 'minimal' }
 * @returns {string} HTML string for social sharing buttons
 */
export function SocialShare(article, options = {}) {
    // Set default variant to 'full' if not specified
    ...
    if (variant === 'minimal') {
        // --- RENDER MINIMAL (ICON-ONLY) VERSION ---
        ...
        // Create the new Copy Link button, which is a <button> not an <a>
        ...
        // MODIFIED: Concatenate the copy button FIRST to move it to the left.
        return `<div class="social-share-minimal">${copyLinkButton}${minimalButtons}</div>`;

    } else {
        // --- RENDER FULL (LABELED) VERSION ---
